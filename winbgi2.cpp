...
{
	x0=x0*scale_x+scale_dx;
	y0=y0*scale_y+scale_dy;
	x1=x1*scale_x+scale_dx;
	y1=y1*scale_y+scale_dy;
    POINT line[2];
    line[0].x = x0;
    line[0].y = y0;
    line[1].x = x1;
...
void clearviewport()
{
    ...
    scr.left = 0;
    scr.top = 0; 
    scr.right = view_settings.right-view_settings.left;//+500
...
void scale(double x0, double y0, double x1, double y1) {
	...
	line(sx-mag,my+mag,mx+mag,my+mag);
	{
		...
		sscanf(xstr,"%lf",&dx);
		for (int i = -floor(-x0/dx); i<=floor(x1/dx); i++) {
			double x = i*dx;
			double tick;
			char xstr[100];
			sprintf(xstr,"%.2lg",x);
	...
	}
	{
		...
		sscanf(ystr,"%lf",&dy);
		for (int i = -floor(-y0/dy); i<=floor(y1/dy); i++) {
			double y = i*dy;
			double tick;
			char ystr[100];
			sprintf(ystr,"%.2lg",y);
